...
for step in range(flags.epochs):
    ...
    for env0, env1, env2 in zip(envs[0], envs[1], envs[2]):
        with tf.GradientTape() as tape_src:
            ...
            print(tf.concat([env[0][0], env[1][0]]).shape)
            train_nll = tf.reduce_mean(tf.concat([env[0][0], env[1][0]]))
            print(train_nll)
            train_accuracy = tf.reduce_mean(tf.concat(env[0][1], env[1][1]))
            train_penalty = tf.reduce_mean(tf.concat(env[0][2], env[1][2]))
