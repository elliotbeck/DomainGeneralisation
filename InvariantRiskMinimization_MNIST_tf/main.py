...
def mean_nll(preds, y):
    return tf.losses.binary_crossentropy(tf.one_hot(tf.cast(y, dtype=tf.int32), depth = 2), preds, from_logits=True)
...
for step in range(flags.epochs):
    ...
    for env0, env1, env2 in zip(envs[0], envs[1], envs[2]):
        with tf.GradientTape() as tape_src:
            ...
            env[0].append(mean_nll(model(env0["image"]), tf.squeeze(env0["label"])))
            env[0].append(mean_accuracy(model(env0["image"]), tf.squeeze(env0["label"])))
            env[0].append(penalty(model(env0["image"]), tf.squeeze(env0["label"])))

            env[1].append(mean_nll(model(env1["image"]), tf.squeeze(env1["label"])))
            env[1].append(mean_accuracy(model(env1["image"]), tf.squeeze(env1["label"])))
            env[1].append(penalty(model(env1["image"]), tf.squeeze(env1["label"])))

            env[2].append(mean_nll(model(env2["image"]), tf.squeeze(env2["label"])))
            env[2].append(mean_accuracy(model(env2["image"]), tf.squeeze(env2["label"])))
            env[2].append(penalty(model(env2["image"]), tf.squeeze(env2["label"])))
