...
for restart in range(flags.n_restarts):
  ...
  class MLP(nn.Module):
    def __init__(self):
      ...
      resnet.fc = nn.Linear(in_ftr, flags.hidden_dim)
  ...
  for step in range(flags.steps):
    for env in envs:
      ...
      permutation = torch.randperm(env['images'].size()[0])
      env['labels'] = env['labels'].squeeze()
      logits = torch.Tensor()
      labels = torch.Tensor()
        
      for i in range(0,env['images'].size()[0], batch_size):
        indices = permutation[i:i+batch_size]
        batch_x, batch_y = env['images'][indices], env['labels'][indices]
        logits = torch.cat((logits.cuda(),mlp(batch_x)), 0)
        labels = torch.cat((labels.cuda(),batch_y.float()), 0)

      logits = logits.float()
      env['nll'] = mean_nll(logits, labels)
