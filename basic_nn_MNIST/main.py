...
DEBUG = False
with open('configs/config_class.json', 'r') as myfile:
    data=myfile.read()
...
def loss_fn(model, features, config, training):
    ...
    classification_loss = tf.losses.binary_crossentropy(
        tf.one_hot(tf.cast(label, dtype=tf.int64), axis=-1, depth=config.num_classes),
        model_output, from_logits=False)
    mean_classification_loss = tf.reduce_mean(classification_loss) 

    accuracy = tf.reduce_mean(
        tf.where(tf.equal(tf.cast(label, dtype=tf.float32), tf.cast(tf.argmax(model_output, axis=-1), dtype=tf.float32)),
...
def main():
    ...
    with writer.as_default(), tf.summary.record_if(lambda: int(global_step.numpy()) % 100 == 0):
        for epoch in range(epoch_start, config.num_epochs):
            ...
            train1_metr = eval_one_epoch(model=model, dataset=ds_train1,
                summary_directory=os.path.join(manager._directory, "train1"), 
                global_step=global_step, config=config, training=False)
            
            train2_metr = eval_one_epoch(model=model, dataset=ds_train2,
